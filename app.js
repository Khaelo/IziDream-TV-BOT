...
const translations = {}
const translationsDir = fs.readdirSync(path.join(__dirname, 'language'))
translationsDir.forEach(i => {
  const name = i.replace('.json', '')
  translations[name] = JSON.parse(fs.readFileSync(path.join(__dirname, 'language', i)))
})
const translate = translations.french
...
function translateDefault (language) {
  ...
  const french = translate

  Object.keys(french).forEach(i => {
    result[i] = JSON.parse(JSON.stringify(french[i]))
    if (lang[i]) result[i] = JSON.parse(JSON.stringify(lang[i]))
  })

  Object.keys(french.commands).forEach(i => {
    result.commands[i] = JSON.parse(JSON.stringify(french.commands[i]))
    if (lang.commands[i]) result.commands[i] = JSON.parse(JSON.stringify(lang.commands[i]))

    Object.keys(french.commands[i]).forEach(ii => {
      result.commands[i][ii] = JSON.parse(JSON.stringify(french.commands[i][ii]))
...
client.once('ready', async () => {
  ...
  client.guilds.cache.forEach(guild => {
    if (!data.guilds[guild.id]) {
      ...
      if (!data.guilds[guild.id].language) saveData([{ guild: guild.id, entry: 'language', value: settings.language || 'french' }])
    }
  })
